(module csl-vector *
  ...
  (define-record-type csl:vector
    ...
    csl:vector?
    (data csl:vector->ptr)
    ;; (real csl:vector-real?)
    )

  (define (choose f1 f2 real)
    (if real f1 f2))
  ...
  (define (csl:vector-add-constant v n)
    (let ((c (csl:vector-copy v)))
      (gsl_vector_add_constant (csl:vector->ptr c) n)
      c))

  (define (csl:vector-add-const! v n)
    (gsl_vector_add_constant (csl:vector->ptr v) (csl:vector->ptr n))
    (void))
